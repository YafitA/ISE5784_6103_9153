...
public class PlaneTests {
    ...
    void testFindGeoIntersectionsHelper() {
        ...
        assertEquals(List.of(new Intersectable.GeoPoint(pl, new Point(1, 0, 0))),
                pl.findGeoIntersections(new Ray(new Point(0.5, 0, 0), new Vector(1, 0, 0)), maxDistance),
                "Bad plane intersection");

        // TC02: Ray does not hit plane
        assertNull(pl.findGeoIntersections(new Ray(new Point(2, 0, 0), new Vector(1, 0, 0)), maxDistance),
                "Must not be plane intersection");

        // =============== Boundary Values Tests ==================
        // TC11: Ray parallel to plane (not on it)
        assertNull(pl.findGeoIntersections(new Ray(new Point(1, 1, 1), new Vector(0, 1, -1)), maxDistance),
                "Must not be plane intersection");

        // TC12: Ray on plane
        assertNull(pl.findGeoIntersections(new Ray(new Point(0, 0.5, 0.5), new Vector(0, 1, -1)), maxDistance),
                "Must not be plane intersection");

        // TC13: Orthogonal ray into plane
        assertEquals(List.of(new Intersectable.GeoPoint(pl, new Point(1d / 3, 1d / 3, 1d / 3))),
                pl.findGeoIntersections(new Ray(new Point(1, 1, 1), new Vector(-1, -1, -1)), maxDistance),
                "Bad plane intersection");

        // TC14: Orthogonal ray out of plane
        assertNull(pl.findGeoIntersections(new Ray(new Point(1, 1, 1), new Vector(1, 1, 1)), maxDistance),
                "Must not be plane intersection");
        ...
        // TC16: Orthogonal ray from plane
        assertNull(pl.findGeoIntersections(new Ray(new Point(0, 0.5, 0.5), new Vector(1, 1, 1)), maxDistance),
                "Must not be plane intersection");

        // TC17: Ray from plane
        assertNull(pl.findGeoIntersections(new Ray(new Point(0, 0.5, 0.5), new Vector(1, 1, 0)), maxDistance),
                "Must not be plane intersection");

        // TC18: Ray from plane's Q point
        assertNull(pl.findGeoIntersections(new Ray(new Point(0, 0, 1), new Vector(1, 1, 0)), maxDistance),
                "Must not be plane intersection");
    }
}